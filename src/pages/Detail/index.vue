          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active: spuSaleAttrValue.isChecked == 0}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id" @click="changeActive(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.trim="skuNum" @change="changeSkuNum"/>
...
export default {
  ...
  methods: {
    async addShopcar() {
      try {
        //由于addOrUpdateShopCart函数调用返回一个Promise对象,我们可以用await拿到函数返回的Promise的结果
        await this.$store.dispatch("addOrUpdateShopCart", {skuId:this.$route.params.skuid, skuNum: this.skuNum})
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo))
        this.$router.push({name: "addChartSuccess", query: {'skuNum':this.skuNum}})
      } catch (error) {
        //抛出错误,error.message可以拿到错误信息
        alert(error.message)
      }
    ...
    changeActive(saleAttrValue, spuSaleAttrValueList) {
      console.log(saleAttrValue, spuSaleAttrValueList)
      spuSaleAttrValueList.map((item, idx) => {
        item.isChecked = 1
        if(saleAttrValue.saleAttrValueName == item.saleAttrValueName) {
